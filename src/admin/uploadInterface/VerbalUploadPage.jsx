...
const VerbalUploadPage = ({ onFileUpload, onCreate }) => {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      // Validate headers
      const requiredHeaders = ["id", "passage", "question", "option1", "option2", "correctAnswer"];
      ...
      const processedQuestions = jsonData.slice(1).map((row, index) => {
        ...
        let correctAnswer = parseInt(rowObj.correctAnswer);
        if (isNaN(correctAnswer)) {
          const match = rowObj.correctAnswer?.toString().match(/\d+/);
        ...
        if (correctAnswer < 0 || correctAnswer >= options.length) {
          setUploadError(`Invalid correctAnswer in row ${index + 2}. Must be between 1 and ${options.length}.`);
          setIsUploading(false);
          return null;
        }

        return {
          id: rowObj.id || index + 1,
